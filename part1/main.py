...
# for i in [.5, 1.0, 2.0]:
#     print('temperature parameter = ',i ,'softmax test_error=', run_softmax_on_MNIST(i),'\n')
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    test_y = np.array([np.remainder(a,3) for a in test_y])
    test_error = compute_test_error_mod3(test_x, test_y, theta, temp_parameter)
    plot_cost_function_over_time(cost_function_history)
    return test_error

    raise NotImplementedError
print('test error mod3 = ',run_softmax_on_MNIST_mod3(temp_parameter=1))
...
def run_softmax_on_MNIST_pca(temp_parameter,n_components):
    """
    Trains softmax, classifies test data, computes test error, and plots cost function
...
train_pca = project_onto_PC(train_x, pcs, n_components, feature_means)
test_pca = project_onto_PC(test_x, pcs, n_components, feature_means)
print('error rate pca =', run_softmax_on_MNIST_pca(1,18))
